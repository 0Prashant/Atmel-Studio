...
/*
 *  Read the value in register of given address of slave
 */
unsigned char twi_ReadRegister(uint8_t slaveAddress_Write, uint8_t registerAddress){

	char tempdata;
	twi_start(slaveAddress_Write);
	twi_write(registerAddress);
	twi_stop();
	twi_start(slaveAddress_Write+1);
	tempdata = twi_read(1);
	twi_stop();
	return tempdata;
}
